...
const EditNote = () => {
  ...
  const loadNote = async (noteId: string) => {
    try {
      ...
      setFormData({
        ...note, // Spread all properties from the fetched note
        imageUrl: note.imageUrl || '' // Ensure imageUrl is always a string
      });
      if (note.imageUrl) {
        setImagePreview(note.imageUrl); // Set image preview if an image exists
      }
    } catch (error: any) {
      alert(error.message || 'ไม่สามารถโหลดบันทึกได้');
      navigate('/dashboard');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await notesAPI.updateNote(id, { ...formData, imageUrl: finalImageUrl });
      navigate('/dashboard');
    } catch (error: any) {
      alert(error.message || 'อัพเดทบันทึกไม่สำเร็จ');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              <span>{(loading || uploadingImage) ? 'กำลังบันทึก...' : 'บันทึกการเปลี่ยนแปลง'}</span>
            </button>
          </div>
        </div>
      </form>
